...
`;function ae(){return k.map(o=>`
  ...
  `).join("")}function ie(){return Object.entries(T).map(([o,t])=>{let e;if(t.type==="number")e=`<input type="number" data-setting="${o}" min="${t.min}" max="${t.max}" />`;else{const a=t.options.map(s=>`<option value="${s.value}">${s.label}</option>`).join("");e=`<select data-setting="${o}">${a}</select>`}return`
      <div class="setting">
